        ...
        {todos.map((todo) => (
          <div
            ...
            className={`flex items-center justify-between mb-4 p-4 border rounded-md ${
              todo.complete ? 'bg-green-100 border-green-300' : 'bg-white border-gray-300'
            }`}
          >
            <div>
              <p className={`${todo.complete ? 'line-through text-gray-500' : 'text-black'}`}>
                {todo.text}
              </p>
              <p className="text-sm text-gray-500">{todo.priority} - {todo.deadline}</p>
            </div>
            <div className="flex items-center">
              <input
                type="checkbox"
                checked={todo.complete}
                onChange={() => handleToggleComplete(todo._id, todo.complete)}
                className="mr-4"
              />
              <button
            ...
            <form onSubmit={handleUpdateTodo}>
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCurrentTodo({ ...currentTodo, complete: e.target.checked })
                  }
                  className="mr-2"
                />
                <span>{currentTodo.complete ? 'Completed' : 'Incomplete'}</span>
              </div>
      ...
      {deleteModalOpen && currentTodo && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <p className="mb-4">
              Are you sure you want to delete the task: <strong>{currentTodo.text}</strong>?
            </p>
            <div className="flex justify-end">
              <button
